...
class MediaVC: VcWithBackGround {
    ...
    let btnPrevious = UIButton() --> {
        ...
        $0.makeWhite()
        $0.layer.cornerRadius = UIDevice.pad ? 45:20
    }
    ...
    let mediaDate = UILabel() --> {
        ...
        $0.layer.cornerRadius = UIDevice.pad ? 45:20
        $0.textColor = .white
        $0.font = .custom(type: .glSemiBold, size: UIDevice.pad ? 27:20)
    ...
    func prepareData(index: Int,data: DialogueModel) {
        ...
        view.addSubview(colletionMedia)
        colletionMedia.snp.makeConstraints { make in
    ...
    func stopPLayer() {
        for case let cell as MediaCell in colletionMedia.visibleCells {
            cell.stopVideo()
        }
    }
...
private extension MediaVC {
    ...
    func prepareSubviews() {
        ...
        mediaDate.snp.makeConstraints { make in
            make.top.equalTo(colletionMedia.snp.bottom).offset(UIDevice.pad ? 40:20)
            make.height.equalTo(UIDevice.pad ? 90:40)
        ...
        activity.snp.makeConstraints { make in
            ...
            make.size.equalTo(UIDevice.pad ? 90:40)
            make.bottom.equalToSuperview().inset(UIDevice.pad ? 80:40)
            
        }
        view.addSubview(btnPrevious)
        btnPrevious.snp.makeConstraints { make in
            make.left.equalToSuperview().inset(UIDevice.pad ? 120:16)
            make.size.equalTo(UIDevice.pad ? 90:40)
        ...
        btnNext.snp.makeConstraints { make in
            make.right.equalToSuperview().inset(UIDevice.pad ? 120:16)
            make.size.equalTo(UIDevice.pad ? 90:40)
            make.bottom.equalTo(activity)
    ...
    func makeLayout() -> UICollectionViewLayout {
        ...
        let topSectionGroupSize = NSCollectionLayoutSize(widthDimension: 
                .absolute((view.frame.width - (UIDevice.pad ? 240:32))*CGFloat(mediaData?.mediaPaths.count ?? 1)),
